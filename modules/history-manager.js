...
// Export conversations to a file (using chrome.downloads API)
export async function exportConversationsToFile() {
  try {
    ...
    return { success: true, count: exportData.count };
  } catch (error) {
    console.error('Error exporting conversations to file:', error);
    throw error;
  }
}

// Import conversations from a file
export async function importConversationsFromFile(file, mergeStrategy = 'skip') {
  try {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const results = await importConversations(data, mergeStrategy);
          resolve(results);
        } catch (error) {
          reject(error);
        }
    ...
    console.error('Error importing conversations from file:', error);
    throw error;
  }
}
...
const AUTO_BACKUP_KEY = 'chatHistoryAutoBackups';
const MAX_AUTO_BACKUPS = 10;

// Create an automatic backup to chrome.storage.local
export async function createAutoBackup() {
  try {
    ...
    let backups = result[AUTO_BACKUP_KEY];

    // Add new backup
    backups.unshift(backup);

    // Keep only the latest MAX_AUTO_BACKUPS backups
    if (backups.length > MAX_AUTO_BACKUPS) {
      backups = backups.slice(0, MAX_AUTO_BACKUPS);
    }

    // Save to storage
    await chrome.storage.local.set({ [AUTO_BACKUP_KEY]: backups });
...
export async function getAutoBackups() {
  try {
    ...
    console.error('Error getting auto backups:', error);
    return [];
  }
}

// Restore conversations from an auto backup
export async function restoreFromAutoBackup(backupId, mergeStrategy = 'skip') {
  try {
    ...
    const results = await importConversations(backup.data, mergeStrategy);
    return results;
  } catch (error) {
...
export async function deleteAutoBackup(backupId) {
  try {
    ...
    console.error('Error deleting auto backup:', error);
    throw error;
  }
}

// Clear all auto backups
export async function clearAllAutoBackups() {
  try {
    await chrome.storage.local.set({ [AUTO_BACKUP_KEY]: [] });
    return { success: true };
  } catch (error) {
...
// Auto backup interval (in milliseconds) - default 1 hour
...
export function startAutoBackup() {
  ...
  console.log('Auto backup started with interval:', AUTO_BACKUP_INTERVAL / 1000 / 60, 'minutes');
}
